...
def simulate_room1():
    while True:
        ...
        try:
            response = requests.get(f"{SERVER_URL}/update_data/room1")
            if response.status_code == 200:
                print(f"Room 1 - Temperature: {temperature}, Fan: {fan_state}")
            else:
                print(f"Failed to update data for Room 1: {response.status_code}")
        except Exception as e:
            print(f"Error sending data for Room 1: {e}")
